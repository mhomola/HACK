...
from Subsystem_design.Wing.inertia import Inertia_normal
...
class Loads_w(Constants):
    ...
    def compute_loads(self):
        ...
        self.W_body = (
                              self.MZFW_320neo - self.Wing_Weight_320neo - 2 * self.W_engine - 2 * self.pod_tank_mass) * self.g_0
        self.WbS = self.W_body / (self.S - S_exp)
        self.WwS = (self.Wing_Weight_320neo - 2*self.W_sharklets) * self.g_0 / self.S
    ...
    def mc_step(self, dist, i):
        return max(0, dist - self.dx / 20) ** (i + 1) / (dist - self.dx / 20)
    ...
    def chord_integrals(self):
        ...
        def c2int1(x):
            return self.m1 * (
                    x ** 3 / 6 - self.width_f ** 2 / 8 * x + self.width_f ** 3 / 16 - self.width_f ** 3 / 48) + \
        ...
        def c2int3(x):
            return self.m2 * (x ** 3 / 6 - self.b_in ** 2 / 8 * x + self.b_in ** 3 / 16 - self.b_in ** 3 / 48) + \
                   (self.c_kink_out - self.m2 * self.b_in / 2) * (
                           x ** 2 / 2 - self.b_in / 2 * x + self.b_in ** 2 / 4 - self.b_in ** 2 / 8)

        def c_sq_int1(x):
            return self.m1 ** 2 * (x ** 3 / 3 - self.width_f ** 3 / 24) + self.m1 * self.c_root * (
                    x ** 2 - self.width_f ** 2 / 4) \
                   + self.c_root ** 2 * (x - self.width_f / 2)

        def c_sq_int2(x):
            return self.m1 ** 2 * (x ** 3 / 3 - self.b_in ** 3 / 24) + self.m1 * self.c_root * (
                    x ** 2 - self.b_in ** 2 / 4) \
        ...
        def cxint1(x):
            return self.m1 * (x ** 3 / 3 - self.width_f ** 3 / 24) + self.c_root * (x ** 2 / 2 - self.width_f ** 2 / 8)

        def cxint2(x):
            return self.m1 * (x ** 3 / 3 - self.b_in ** 3 / 24) + self.c_root * (x ** 2 / 2 - self.b_in ** 2 / 8)

        def cxint3(x):
            return self.m2 * (x ** 3 / 3 - self.b_in ** 3 / 24) \
                   + (self.c_kink_out - self.m2 * self.b_in / 2) * (x ** 2 / 2 - self.b_in ** 2 / 8)
    ...
    def Reaction_forces(self):
        ...
        self.RF_y = spint.simps(y=(L_arr - Ww_arr), x=x_arr) - self.W_engine * self.g_0 \
                    - self.pod_tank_mass * self.g_0 - self.W_sharklets * self.g_0
    ...
    def S_y(self, x):
        Sy = - self.RF_y * self.mc_step(dist=x - 0.5 * self.width_f, i=0) \
             ...
             - self.W_sharklets * self.g_0 * self.mc_step(dist=x - self.b / 2, i=0)
        return Sy
    ...
    def M_x(self, x):
        Mx = - self.RM_x * self.mc_step(dist=x - 0.5 * self.width_f, i=0) \
             ...
             - self.W_sharklets * self.g_0 * self.mc_step(dist=x - self.b / 2, i=1)
        return Mx
    ...
    def eng_torque_arm(self, x):
        """
        The distance from the cg of the engine to the centroid of the wingbox. 1.62 m is the distance from the
        cg of the engine to the LE, and then 0.415 is the distance from the LE to the centroid of the wingbox over the
        local chord length.
        :param x: distance from plane of symmetry towards the tip
        :return:
        """
        inertia = Inertia_normal(n_str=10)  # n_str is just a random value, does  not really matter
        c = inertia.chord_inertia(x=x)
    ...
    def tank_torque_arm(self, x):
        """
        The distance from the cg of the podded tank to the centroid of the wingbox. 11.88 m is the distance from
        the nose to the LE of the wing at the root.
        :param x:
        :return:
        """
        inertia = Inertia_normal(n_str=10)  # n_str is just a random value, does  not really matter
        ...
        self.W_tank_arm = self.x_cg_pod - (11.88 + np.tan(self.sweep_LE * np.pi / 180) * x + inertia.pos_centroid_c * c)

    def Reaction_torque(self):
        x = self.b / 2
        self.eng_torque_arm(x=x)
        self.tank_torque_arm(x=x)

        inertia = Inertia_normal(n_str=10)  # n_str is just a random value, does  not really matter
        c = inertia.chord_inertia(x=x)
        w_sk = inertia.w_sk_c(x=x) * c
        h_spar = inertia.t_c_distribution(x=x) * c
        centroid_at_x = inertia.pos_centroid_c * c

        w_sk_c = inertia.w_sk_c(x=self.width_f / 2)
        centroid_at_wf = inertia.pos_centroid_c * inertia.chord_inertia(x=self.width_f / 2)

        self.RT = - self.RF_y * (np.tan(self.sweep_LE * np.pi / 180) * x + centroid_at_x - centroid_at_wf) \
                  * self.mc_step(dist=x - self.width_f / 2, i=0) \
            + self.C_L_crit * self.q_crit * \
            (-0.25 * (self.c_sq_int1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                      - self.c_sq_int2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
                      + self.c_sq_int3(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0))
             - np.tan(self.sweep_LE * np.pi / 180) * (
                     self.cxint1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                     - self.cxint2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
                     + self.cxint3(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0))
             + (centroid_at_x + np.tan(self.sweep_LE * np.pi / 180) * x)
             * (self.cint1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                - self.cint2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
                + self.cint3(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0))) \
            + (self.max_T - self.D_eng) * self.T_arm * self.mc_step(dist=x - self.y_engine, i=0) \
            - self.W_engine * self.g_0 * self.W_eng_arm * self.mc_step(dist=x - self.y_engine, i=0) \
            + self.pod_tank_mass * self.g_0 * self.W_tank_arm * self.mc_step(dist=x - self.y_cg_pod, i=0) \
            + self.D_tank_sys * (self.pylon_height + d_wing_pod / 2 + h_spar / 2 * self.chord(x=self.y_cg_pod)) * \
            self.mc_step(dist=x - self.y_cg_pod, i=0) \
            + self.WwS * \
            (-0.5 * (self.c_sq_int1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                     - self.c_sq_int2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
                     + self.c_sq_int3(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0))
             - np.tan(self.sweep_LE * np.pi / 180) * (
                      self.cxint1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
             ...
             * (self.cint1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                - self.cint2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
                + self.cint3(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0))) \
            + self.W_sharklets * self.g_0 * 0.5 * self.mc_step(dist=x - self.b / 2, i=0)
    ...
    def T_z(self, x):
        ...
        Tz = - self.RT * self.mc_step(dist=x - self.width_f / 2, i=0) \
            - self.RF_y * (np.tan(self.sweep_LE * np.pi / 180) * x + centroid_at_x - centroid_at_wf) \
             * self.mc_step(dist=x - self.width_f / 2, i=0) \
            + self.C_L_crit * self.q_crit * \
             (-0.25 * (self.c_sq_int1(x=x) * self.mc_step(dist=x - 0.5 * self.width_f, i=0)
                       - self.c_sq_int2(x=x) * self.mc_step(dist=x - self.b_in / 2, i=0)
